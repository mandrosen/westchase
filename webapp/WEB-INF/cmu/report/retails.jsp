...
<h1>Westchase CMU Report - Retail Centers</h1>

<s:form name="cmuRetailForm" action="retail" id="frm" theme="simple">
...
<tbody>
    <s:iterator value="results" status="status" id="result">
        <tr class="<s:if test="#status.even">even</s:if><s:else>odd</s:else> <c:if test="${not empty verified}">verified</c:if>">
            ...
            <td><a href="<s:url action="officeRetailSvcEdit-%{id}-2" />"><s:property value="id"/></a></td>
    ...
    <c:if test="${total > 0}">
	    <tr>
	    	...
	    	<td><strong><fmt:formatNumber maxFractionDigits="2" value="${occRate / total}" />%</strong></td>
	    	<td colspan="7">&#160;</td>
	    </tr>
    </c:if>
